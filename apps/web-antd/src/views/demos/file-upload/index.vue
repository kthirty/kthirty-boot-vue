  ...
  schema: [
    {
      ...
      renderComponentContent: () => ({
        default: () => h(Button, { type: 'primary' }, () => $t('ui.upload.select')),
      }),
    },
    {
      component: 'OosFile',
...
async function handleDownload(file: OosFileApi.OosFileVO) {
  ...
  >
    ...
    <Card :title="$t('demos.fileUpload.multiTitle')" class="mt-4">
      <Upload v-model:file-list="multiFileList" v-bind="multiUploadProps">
        <Button type="primary">{{ $t('demos.fileUpload.selectMultiple') }}</Button>
      </Upload>
